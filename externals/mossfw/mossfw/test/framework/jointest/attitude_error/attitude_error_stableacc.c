...
#endif
  ret = mossfw_deliver_data(getattitude_errorpipeline()->stable_acc.out[0], &acc);
  assert(ret == 0);
  ret = mossfw_deliver_data(getattitude_errorpipeline()->stable_acc.out[1], &stable);
  assert(ret == 0);

  return ret;
}

void attitude_error_stable_acc_create(struct component_s *com)
{
  if (com != NULL)
    {
      com->in_num  = 1;
...
void attitude_error_stable_acc_delete(struct component_s *com)
{
