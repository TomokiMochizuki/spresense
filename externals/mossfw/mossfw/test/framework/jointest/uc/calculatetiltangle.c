  ...
  if (valid == 1)
    {
      ...
      xdeg = xrad * 180.0f / PI_FLOAT;
      ydeg = yrad * 180.0f / PI_FLOAT;
      ...
      mossfw_deliver_dataarray(getucpipeline()->calculatetiltangle.out[0], odata);
      mossfw_data_free(odata);
    }
  if (1 < valid) 
    {
      mossfw_deliver_dataarray(getucpipeline()->calculatetiltangle.out[1], data);
    }
  if (2 < valid) 
    {
      odata = mossfw_data_alloc(allocator, sizeof(mossfw_data_qf_t), true);
...
void calculatetiltangle_create(struct component_s *com)
{
  if (com != NULL)
    {
      com->in_num  = 1;
...
void calculatetiltangle_delete(struct component_s *com)
{
