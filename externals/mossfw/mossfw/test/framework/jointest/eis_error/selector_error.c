...
#define SELECTOR_INPUTTYPE_GYR  (MOSSFW_DATA_TYPE_FLOAT + \
                                 ...
                                 MOSSFW_DATA_TYPEARRAY_ARRAY)
#define SELECTOR_INPUTTYPE_LINE (MOSSFW_DATA_TYPEARRAY_ARRAY)
...
static int selector_setoffset (mossfw_input_t *input, unsigned long indataid)
{
    int ret;
...
static int selector_operate_cb(struct mossfw_callback_op_s *in, unsigned long indataid)
{
    int8_t ret = 0;
    int datasizeacc  = 0;
    int datasizegyr  = 0;
    ...
    size_t numgyr  = 0;
    size_t numline = 0;
    mossfw_input_t *inputacc  = NULL;
    mossfw_input_t *inputgyr  = NULL;
    mossfw_input_t *inputline = NULL;
    ...
    if (indataid == SELECTOR0)
      {
        outputacc   = geteis_errorpipeline()->selector.out[SELECTOR0_ACC];
        outputgyr   = geteis_errorpipeline()->selector.out[SELECTOR0_GYR];
        outputline  = geteis_errorpipeline()->selector.out[SELECTOR0_LINE];
      }
    else
      {
        outputacc   = geteis_errorpipeline()->selector.out[SELECTOR1_ACC];
        outputgyr   = geteis_errorpipeline()->selector.out[SELECTOR1_GYR];
        outputline  = geteis_errorpipeline()->selector.out[SELECTOR1_LINE];
      }

    datasizeacc  = mossfw_get_delivereddata_num(inputacc);
    datasizegyr  = mossfw_get_delivereddata_num(inputgyr);
    datasizeline = mossfw_get_delivereddata_num(inputline);
    indataacc  = mossfw_get_delivereddata_array(inputacc, datasizeacc, &used);
    assert(indataacc != NULL);
    numacc  = (size_t)indataacc->data_bytes  / sizeof(mossfw_data_v3f_t);
    indatagyr  = mossfw_get_delivereddata_array(inputgyr, datasizegyr, &used);
    assert(indatagyr != NULL);
    numgyr  = (size_t)indatagyr->data_bytes  / sizeof(mossfw_data_v3f_t);
    indataline = mossfw_get_delivereddata_array(inputline, datasizeline, &used);
    assert(indataline);
    numline = (size_t)indataline->data_bytes / sizeof(struct resampleinfo_s);
    if (indataid == SELECTOR0) {
#ifdef DEBUG_OUTPUT
