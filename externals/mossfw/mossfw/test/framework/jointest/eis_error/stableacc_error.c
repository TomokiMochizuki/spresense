...
static int stableacc_operate_cb(struct mossfw_callback_op_s *in, unsigned long indataid)
{
    ...
    ret = mossfw_deliver_dataarray(geteis_errorpipeline()->stableacc.out[0], indata);
    assert(ret == 0);
    ret = mossfw_deliver_dataarray(geteis_errorpipeline()->stableacc.out[1], stablemem);
    assert(ret == 0);
...
void stableacc_error_create(struct component_s *com)
{
  if (com != NULL)
    {
      com->in_num  = 1;
...
void stableacc_error_delete(struct component_s *com)
{
