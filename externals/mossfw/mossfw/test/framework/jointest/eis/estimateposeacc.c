...
static int estimateposeacc_operate_cb(struct mossfw_callback_op_s *in, unsigned long indataid)
{
    ...
    #endif
            ret = mossfw_deliver_dataarray(geteispipeline()->estimateposeacc.out[0], outmem);
            assert(ret == 0);
        }
        else
          {
            ret = mossfw_deliver_dataarray(geteispipeline()->estimateposeacc.out[0], indatastable);
            assert(ret == 0);
          }
      }
    else
...
void estimateposeacc_create(struct component_s *com)
{
  if (com != NULL)
    {
      com->in_num  = 2;
      ...
      mossfw_set_waitcondition(com->in[1], 1, com->op[0]);
    }
}

void estimateposeacc_delete(struct component_s *com)
{
