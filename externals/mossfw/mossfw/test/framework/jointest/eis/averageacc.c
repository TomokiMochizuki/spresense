...
int averageacc_operate_cb(struct mossfw_callback_op_s *in, unsigned long indataid)
{
    ...
    if (count == 3) {
        ...
        fflush(stdout);
#endif
        ret = mossfw_deliver_dataarray(geteispipeline()->averageacc.out[0], indata);
        assert(ret == 0);
    }
    else
      {
...
void averageacc_create(struct component_s *com)
{
  bool async = true; 
  if (com != NULL)
    {
...
void averageacc_delete(struct component_s *com)
{
