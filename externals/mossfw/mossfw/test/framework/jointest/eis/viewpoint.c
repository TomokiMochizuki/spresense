...
#endif
        ret = mossfw_deliver_dataarray(geteispipeline()->viewpoint.out[0], indata);
        assert(ret == 0);
    }
...
void viewpoint_create(struct component_s *com)
{
  if (com != NULL)
    {
      com->in_num  = 1;
...
void viewpoint_delete(struct component_s *com)
{
