...
static int transform_operate_cb(struct mossfw_callback_op_s *in, unsigned long indataid)
{
    ...
    if (indataid == TRANSFORM_ACC0)
      {
        output = geteispipeline()->transformacc0.out[0];
      }
    else if (indataid == TRANSFORM_GYR0)
      {
        output = geteispipeline()->transformgyr0.out[0];
      }
    else if (indataid == TRANSFORM_ACC1)
      {
        output = geteispipeline()->transformacc1.out[0];
      }
    else
      {
        output = geteispipeline()->transformgyr1.out[0];
      }
...
void transform_create(struct component_s *com, enum transform_e type)
{
  if (com != NULL)
    {
      com->in_num    = 1;
...
void transform_delete(struct component_s *com)
{
    int i;
