...
#define EISSINK_INPUTTYPE_GYR  (MOSSFW_DATA_TYPE_FLOAT + \
                                ...
                                MOSSFW_DATA_TYPEARRAY_ARRAY)
#define EISSINK_INPUTTYPE_INDEX (MOSSFW_DATA_TYPE_INT32 + \
                                 MOSSFW_DATA_TYPEGRP_V3 + \
                                 MOSSFW_DATA_TYPEARRAY_ARRAY)
#define EISSINK_INPUTTYPE_LINE (MOSSFW_DATA_TYPE_INT32 + \
...
static void resampleinfo_expect(mossfw_data_t *indexdata, mossfw_data_t *linedata, struct resampleinfo_s *exp, size_t *count)
{
    mossfw_data_v3i32_t *dataindex = indexdata->data.xyzi32;
    mossfw_data_v2i32_t  *dataline  = linedata->data.xyi32;
    int numindex = indexdata->data_bytes / sizeof(mossfw_data_v3i32_t);
    int numline  = linedata->data_bytes  / sizeof(mossfw_data_v2i32_t);
    assert(numindex == numline);
    for (int i = 0; i < numindex && i < numline; i++)
      {
          assert(dataindex[i].x == exp[*count + i].slerpflg);
          assert(dataindex[i].y == exp[*count + i].q1idx);
          assert(dataindex[i].z == exp[*count + i].q2idx);
          assert(dataline[i].x  == exp[*count + i].line1);
          assert(dataline[i].y  == exp[*count + i].line2);
      }
  ...
  if (arg == 0)
    {
      ...
      inputindex = inputgyr->cb_next;
      inputline  = inputindex->cb_next;
      sizeacc   = mossfw_get_delivereddata_num(inputacc);
      sizegyr   = mossfw_get_delivereddata_num(inputgyr);
      sizeindex = mossfw_get_delivereddata_num(inputindex);
  ...
  if (0 < sizeacc)
    {
      ...
      assert(indataacc);
      vec3_expect(indataacc, acc_expected, &nacc);
    }
  if (0 < sizegyr)
    {
      indatagyr = mossfw_get_delivereddata_array(inputgyr, sizegyr, &usedgyr);
      assert(indatagyr);
      vec3_expect(indatagyr, gyr_expected, &ngyr);
    }
  if (0 < sizeindex && 0 < sizeline)
    {
      indataindex = mossfw_get_delivereddata_array(inputindex, sizeindex, &usedindex);
      assert(indataindex);
      indataline  = mossfw_get_delivereddata_array(inputline, sizeline, &usedline);
      assert(indataline);
      if (1 < sizeindex && 1 < sizeline)
        {
          resampleinfo_expect(indataindex, indataline, resampleinfo_expected, &nline);
        }
    }
  if (0 < sizerot)
    {
      ...
      assert(indatarot);
      rot_expect(indatarot, rot_expected, &nrot);
    }
...
void eissink_create(struct component_s *com)
{
  if (com != NULL)
    {
      ...
      com->in[EISSINK_INDEX] = mossfw_input_create(EISSINK_INPUTTYPE_INDEX, 2);
      ...
      mossfw_set_waitcondition(com->in[EISSINK_INDEX],   1,                            com->op[0]);
...
void eissink_delete(struct component_s *com)
{
    component_delete(com);
}

/* return 0 is succeeded */
int eissink_succeeded(void)
{
    int accerr;
    int gyrerr;
    int lineerr;
    ...
    accerr  = (nacc  != sizeof_acc_expected() / (sizeof acc_expected[0]));
    gyrerr  = (ngyr  != sizeof_gyr_expected() / (sizeof gyr_expected[0]));
    lineerr = (nline != sizeof_resampleinfo_expected() / (sizeof resampleinfo_expected[0]));
    roterr  = (nrot  != sizeof_rot_expected() / (sizeof rot_expected[0]));
