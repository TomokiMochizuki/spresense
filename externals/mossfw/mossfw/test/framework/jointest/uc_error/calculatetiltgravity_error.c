  ...
  if (0 < sizeacc)
    {
      ...
      mossfw_deliver_dataarray(getuc_errorpipeline()->calculatetiltgravity.out[0], data);
      mossfw_data_free(data);

      data = mossfw_data_alloc(allocatorf, sizeof(mossfw_data_v1f_t), true);
      assert(data);
      data->data.v1f[0].x = out_v.theta_angle;
      data->data_bytes = sizeof data->data.v1f[0];
      mossfw_deliver_dataarray(getuc_errorpipeline()->calculatetiltgravity.out[1], data);
      mossfw_data_free(data);
    }
  return 0;
}

void calculatetiltgravity_error_create(struct component_s *com)
{
  int val;
...
void calculatetiltgravity_error_delete(struct component_s *com)
{
