  ...
  if (arg == LPF_ACC)
    {
      param = &accparam;
      count = &acccount;
      output = getattitudepipeline()->lpf_acc.out[0];
    }
  else // arg == LPF_GYR
    {
      param = &gyrparam;
      count = &gyrcount;
      output = getattitudepipeline()->lpf_gyr.out[0];
    }
...
void attitude_lpf_reset(void)
{
  acccount = 0;
  gyrcount = 0;
}

void attitude_lpf_create(struct component_s *com, enum lpf_e type)
{
  if (com != NULL)
    {
      com->in_num    = 1;
...
void attitude_lpf_delete(struct component_s *com)
{
