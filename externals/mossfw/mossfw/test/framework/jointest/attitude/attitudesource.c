...
static int attitudesource_operate_cb(struct mossfw_callback_op_s *in, unsigned long arg)
{
  int ret = 0;
  int i;
  int j;

  attitude_complementaryfilter_set_time_resolution(100);

  for (i = 0; i < data10_num(); i++)
    {
      mossfw_deliver_data(getattitudepipeline()->attitudesource.out[1], gyr_data10(i));
      mossfw_deliver_data(getattitudepipeline()->attitudesource.out[0], acc_data10(i));
    }
  attitude_lpf_reset();
  for (i = 0; i < data14_num(); i++)
    {
      mossfw_deliver_data(getattitudepipeline()->attitudesource.out[1], gyr_data14(i));
      mossfw_deliver_data(getattitudepipeline()->attitudesource.out[0], acc_data14(i));
    }
  ...
  if (com != NULL)
    {
      ...
      *com->op = mossfw_callback_op_create(attitudesource_operate_cb, 0, false);
    }
}
