...
static int result_operate_cb(struct mossfw_callback_op_s *in, unsigned long arg)
{
  ...
  rot_expect(&data, attitude_rot_expected, &nrot);

  return ret;
}

void attituderesult_create(struct component_s *com)
{
  if (com != NULL)
    {
      com->in_num  = 1;
...
void attituderesult_delete(struct component_s *com)
{
  component_delete(com);
}

int attituderesult_succeeded(void)
{
  int roterr;

  roterr  = (nrot  != attitude_sizeof_rot_expected() / (sizeof attitude_rot_expected[0]));
  return roterr;
}
